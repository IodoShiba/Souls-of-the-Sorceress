...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            string GetTargetControl(InputBinding binding)
            {
                if (action.type == InputActionType.Button)
                {
                    return binding.path.Contains("Keyboard") ? "<Keyboard>" :
                    binding.path.Contains("Gamepad") ? "<Gamepad>" :
                    "";
                }
                else if (action.type == InputActionType.Value && (action.expectedControlType == "Stick" || action.expectedControlType == "Vector2"))
                {
                    return binding.isComposite ? "<Keyboard>" : "<Gamepad>";
                }
                else
                {
                    return "";
            ...
            action.Disable();
            var targetControl = GetTargetControl(action.bindings[bindingIndex]);
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex).WithControlsHavingToMatchPath(targetControl) // FIXME:
                .OnCancel(
                    operation =>
                    {
            ...
            // m_RebindOverlay?.SetActive(true);s
            ...
            // if (m_RebindOverlay == null && m_RebindText == null && m_RebindStartEvent.GetPersistentEventCount() == 0 && m_BindingText != null)
            if (m_RebindOverlay == null && m_RebindText == null && m_BindingText != null)
                m_BindingText.text = "<“ü—Í‘Ò‚¿...>";

            // Give listeners a chance to act on the rebind starting.
            m_RebindStartEvent?.Invoke(this, m_RebindOperation);

            m_RebindOperation.WithTimeout(5.0f);
